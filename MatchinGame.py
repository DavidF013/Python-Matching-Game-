...
def reset_game():
    ...
    # makes sure that the number and rows are always even
    if (rows * cols) % 2 != 0:
        # If its odd,  add an extra column
        cols += 1
        print(f"Adjusted to {rows} rows and {cols} columns (even total).")
    ...
    while len(ids) > 0:
        a = ids.pop(randint(0, len(ids) - 1))
        b = ids.pop(randint(0, len(ids) - 1))
